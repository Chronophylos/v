...
fn new() {
    ...
    let mut response = client
        ...
        .body("title=title&url=https%3A%2F%2Fi.imgur.com%2FVoyouQH.png")
        .dispatch();
...
fn new_invalid_image_url() {
    ...
    let mut response = client
        .post("/a/new")
        .header(ContentType::Form)
        .body("title=title&url=https%3A%2F%2Fi.evil.com%2FVoyouQH.png")
        .dispatch();
...
fn import() {
    ...
    let mut response = client
        .post("/a/import")
        .header(ContentType::Form)
        .body("title=&url=https%3A%2F%2Fimgur.com%2Fa%2FJrheYnV")
        .dispatch();
...
#[test]
#[ignore = "not final, missing api to check if ressouces were correctly added"]
fn create_album_and_insert() {
    let client = Client::new(v::rocket()).expect("valid rocket instance");
